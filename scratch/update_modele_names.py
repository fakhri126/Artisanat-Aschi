...
print(f"Total Buffets to update: {len(buffets_sorted)}")

# We assign luxury prestige model codes: "Buffet — Modèle 01", "Buffet — Modèle 02", etc.
for idx, p in enumerate(buffets_sorted, start=1):
    ...
    num_str = f"{idx:02d}"
    pro_name = f"Buffet — Modèle {num_str}"
    
    payload = {
        "name": pro_name,
        "description": f"Buffet artisanal d'art fait-main sur-mesure (Modèle {num_str}) — Finition {color}, format {dim}.",
        "categoryId": cat_id,
        "dimensions": dim,
...
print("\n[SUCCESS] All buffets updated with luxury 'Buffet — Modèle XX' names!")
